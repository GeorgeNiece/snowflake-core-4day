-- 1.6.4 Set the context for worksheet 
USE ROLE ACCOUNTADMIN;
USE CITIBIKE_PIPELINES.PUBLIC;
USE WAREHOUSE DATAPIPELINES_WH;

-- 1.6.4 Create Cloud Storage Integration
...
-- 1.6.5 Retrieve the AWS IAM User for your Snowflake Account and 
...
-- 2.1.1 Create a table that Snowpipe will use to write the incoming data. 
create or replace table trips_raw (v variant);

-- 2.1.2 Create a SNOW PIPE definition
create or replace pipe trips_pipe auto_ingest=true as copy into trips_raw from @streaming_data/;

-- 2.1.3 Review the Snow pipe definition and make a note of the notification channel value.
show pipes;



-- 2.1.5 Call the Stored procedure that generates data files
call stream_data('2018-01-01', '2018-01-01');

-- 2.1.5 Verify if there are any files already in the bucket/folder
list @streaming_data;

-- 2.1.5 Check the status of the pipe. 
select system$pipe_status('trips_pipe');

-- 2.1.5 Check if the data is loaded to the trips_raw table. 
select count(*) from trips_raw;

-- 2.1.5 Review sample data from trips_raw table. 
select * from trips_raw limit 100;

-- 2.1.5 Clean up the stage by calling the purge_files function.  
call purge_files('trips_raw', '@streaming_data/');

-- 2.1.5 Truncate raw table
truncate table trips_raw;


-- 2.3.3 Check the information_schema.copy_history to see if there were any problems encountered while writing records to the trips_raw table.
select *
...
-- 3.1.1 Create Streams for trips and stations.
...
-- 3.1.2 Load 1 day of data to test the streams.
...
-- 3.1.4 Check the status of the pipe and watch for file create events
select system$pipe_status('trips_pipe');

-- 3.1.5 Snowpipe copies the data into the raw table and the insertions are tracked in the stream.
...
-- 3.1.7 Create tables to store the data processed by the Streams.
...
-- 4.1.1 Create the push_trips task to read JSON data in the streams_trips stream and load to bike_trips
...
-- 4.1.2 Create a task to perform a merge into the bike_stations table.
...
-- 4.1.3 Define a TASK to call the purge_files procedure AFTER the push_trips TASK runs.
...
-- 4.1.4 Activate the tasks that were just created.
...
-- 4.1.5 Verify data loaded from streams to the tables.
select count(*) from citibike_pipelines.public.bike_trips;
select count(*) from citibike_pipelines.public.bike_stations;

-- 5.1.1 Call the stream_data procedure to drop files into the STAGE for couple of days
call stream_data('2018-01-03', '2018-01-04');

-- 5.1.2 Show the details of each of the tasks over the last five minutes.
select * from table(information_schema.task_history())
  where scheduled_time > dateadd(minute, -5, current_time())
  and state <> 'SCHEDULED'
  order by completed_time desc;

-- 5.1.3 How long until the next task runs?
...
-- 5.1.4 How many files have been processed by the pipeline in the last hour?
select count(*)
from table(information_schema.copy_history(
  table_name=>'citibike_pipelines.public.trips_raw',
  start_time=>dateadd(hour, -1, current_timestamp)));
...
-- 5.1.5 query to get an overview of the pipeline including: time to next task run, the number of files in the bucket, 
...
-- 5.1.6 Suspend the tasks using the below queries.
